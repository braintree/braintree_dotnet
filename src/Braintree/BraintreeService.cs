...
namespace Braintree
{
    public class BraintreeService : HttpService
    {
        ...
        private XmlNode GetXmlResponse(string URL, string method, Request requestBody, Stream fileStream, string fileName)
        {
            if (!IsValidURL(URL))
            {
                throw new ArgumentException($"Path: {URL} is malformed.");
        ...
        private async Task<XmlNode> GetXmlResponseAsync(string URL, string method, Request requestBody, Stream fileStream, string fileName)
        {
            if (!IsValidURL(URL))
            {
                throw new ArgumentException($"Path: {URL} is malformed.");
