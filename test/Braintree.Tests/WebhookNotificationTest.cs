...
namespace Braintree.Tests
{
    ...
    public class WebhookNotificationTest
    {
        ...
        [Test]
        public void SampleNotification_ReturnsANotificationForDisputeUnderReviewWebhook()
        {
            Dictionary<string, string> sampleNotification = gateway.WebhookTesting.SampleNotification(WebhookKind.DISPUTE_UNDER_REVIEW, "my_id");

            WebhookNotification notification = gateway.WebhookNotification.Parse(sampleNotification["bt_signature"], sampleNotification["bt_payload"]);

            Assert.AreEqual(WebhookKind.DISPUTE_UNDER_REVIEW, notification.Kind);
            ...
            Assert.AreEqual(DisputeStatus.UNDER_REVIEW, notification.Dispute.Status);
        ...
        [Test]
        public void SampleNotification_ReturnsANotificationForDisputeAutoAcceptedWebhook()
        {
            Dictionary<string, string> sampleNotification = gateway.WebhookTesting.SampleNotification(WebhookKind.DISPUTE_AUTO_ACCEPTED, "my_id");

            WebhookNotification notification = gateway.WebhookNotification.Parse(sampleNotification["bt_signature"], sampleNotification["bt_payload"]);

            Assert.AreEqual(WebhookKind.DISPUTE_AUTO_ACCEPTED, notification.Kind);
            Assert.AreEqual("my_id", notification.Dispute.Id);
            Assert.AreEqual("my_id", notification.Dispute.TransactionDetails.Id);
            Assert.AreEqual("250.00", notification.Dispute.TransactionDetails.Amount);
            Assert.AreEqual(DisputeStatus.AUTO_ACCEPTED, notification.Dispute.Status);
            Assert.AreEqual(DisputeKind.CHARGEBACK, notification.Dispute.Kind);
            Assert.AreEqual(new DateTime(2014, 3, 21), notification.Dispute.DateOpened);
        }
        ...
        public void SampleNotification_ReturnsANotificationForLocalPaymentCompleted()
        {
          ...
          Assert.IsEmpty(localPayment.BlikAliases);
        ...
        [Test]
        public void SampleNotification_ReturnsANotificationForLocalPaymentCompletedBlikOneClick()
        {
          Dictionary<string, string> sampleNotification = gateway.WebhookTesting.SampleNotification(WebhookKind.LOCAL_PAYMENT_COMPLETED, "blik_one_click_id");
          ...
          Assert.AreEqual("a-bic", localPayment.Bic);
          Assert.AreEqual("unique-key-1", localPayment.BlikAliases[0].Key);
          Assert.AreEqual("unique-label-1", localPayment.BlikAliases[0].Label);
          Assert.AreEqual("1234", localPayment.IbanLastChars);
          Assert.AreEqual("a-payer-id", localPayment.PayerId);
          Assert.AreEqual("a-payer-name", localPayment.PayerName);
          Assert.AreEqual("a-payment-id", localPayment.PaymentId);
